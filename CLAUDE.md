...
**AI Analysis (removed from the frontend 2026-09-13):** The app no longer calls `action=analyze` — the buy-timing verdict in `src/lib/buyTiming.ts` is the analysis, and the "Get AI Buy-Timing Analysis" button, its card and `parseAnalysis()` are gone from `App.tsx`. The Lambda action still exists (with its daily budget) but nothing calls it.
...
- Current: all-event search (SeatGeek discovery), Price Watch tracking + automatic resale price logging (Claude web search → DynamoDB), buy-now-or-wait verdicts from trend + demand + days-out patterns
